...
fun main() {
    ...
    val filterdMap = numberPairs2.filter { (key, value) -> key.endsWith("e") && value > 0 }.sortedBy { it.first }.reversed()
    println("$filterdMap")


    val runs = mapOf( "ST" to 100, "MSD" to 122, "VK" to 180, "HP" to 77)
    val filteredRuns = runs.filter { (key, value) -> value > 100 }.toList().sortedBy { it.second }.reversed()
...
//    a.second = 5
    ...
    println(simpleSum)
    val sumDoubled = numbers9.fold(0) { sum, element -> println("$sum $element"); sum + element * 2 }
    println(sumDoubled)
